...
import sys
import traceback
import GladeScurity.modules.sql.users_sql as sql
from sys import argv
from typing import Optional
from telegram import __version__ as peler
from platform import python_version as memek
from GladeScurity import (
    ALLOW_EXCL,
    CERT_PATH,
    DONATION_LINK,
    LOGGER,
...
from GladeScurity.modules import ALL_MODULES
from GladeScurity.modules.helper_funcs.chat_status import is_user_admin
from GladeScurity.modules.helper_funcs.misc import paginate_modules
...
Glade_IMG = "https://telegra.ph/file/a40e5de294e54b59463ff.jpg"
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("GladeScurity.modules." + module_name)
...
def Glade_about_callback(update, context):
    query = update.callback_query
    if query.data == "Glade_":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="Admins", callback_data="Glade_admin"),
                    InlineKeyboardButton(text="Notes", callback_data="Glade_notes"),
                 ],
                 [
                    InlineKeyboardButton(text="Support", callback_data="Glade_support"),
                    InlineKeyboardButton(text="Credits", callback_data="Glade_credit"),
                 ],
                 [
                    InlineKeyboardButton(text="Musicplayer", callback_data="source_"),
                 ],
                 [
                    InlineKeyboardButton(text="Go Back", callback_data="Glade_back"),
                 ]
                ]
            ),
        )
    elif query.data == "Glade_back":
        first_name = update.effective_user.first_name
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
                PM_START_TEXT.format(
    ...
    elif query.data == "Glade_admin":
        query.message.edit_text(
            text=f"*๏ Let's make your group bit effective now*"
            "\nCongragulations, Glade Mega now ready to manage your group."
            "\n\n*Admin Tools*"
            ...
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(text="Go Back", callback_data="Glade_")]]
            ),
        )

    elif query.data == "Glade_notes":
        query.message.edit_text(
    ...
    elif query.data == "Glade_support":
        query.message.edit_text(
            text=f"*๏ {BOT_NAME} support chats*"
            "\nJoin My Support Group/Channel for see or report a problem on GladeScurity.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(
                [
                 [
    ...
    elif query.data == "Glade_credit":
        query.message.edit_text(
            text=f"<b>๏ Credis for GladeScurity</b>\n"
            f"\nHere Developers Making The GladeScurity",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [
                 [
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(
        Glade_about_callback, pattern=r"Glade_", run_async=True
    )
